...
func (c *cli) RunAll(runStacks []ExecContext, isSuccessCode func(exitCode int) bool) error {
	...
	for i, runContext := range runStacks {
		...
		if !c.parsedArgs.Quiet {
			c.stderrPrinter.Println(printPrefix + " Entering stack in " + runContext.Stack.String())
			c.stderrPrinter.Println(printPrefix + " Executing command " + strconv.Quote(cmdStr))
		}
